...
import { TenXScreenerClient } from "@/components/tenx-screener-client";
import { SESSION_COOKIE, validateSessionToken } from "@/lib/auth";
import { getMacroRegimeRadarData, getTenXStockScreenerData } from "@/lib/market";

export const dynamic = "force-dynamic";

export default async function TenXPage() {
  ...
  const [candidates, radar] = await Promise.all([
    getTenXStockScreenerData(),
    getMacroRegimeRadarData(),
  ]);
  ...
  return (
    <main className="min-h-screen bg-[var(--app-bg)] px-4 py-4 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4">
        ...
        <TenXScreenerClient initialCandidates={candidates} radar={radar} />
      </div>
    </main>
  );
}
